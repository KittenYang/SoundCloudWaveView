...
@property (nonatomic, strong) CADisplayLink *playingTimer;
...
- (void)play
{
    if (!self.playing)
    {
        ...
        //play sound
        [_sound play];
        if (self.playingTimer == nil) {
            self.playingTimer = [CADisplayLink displayLinkWithTarget:self selector:@selector(playing)];
            [self.playingTimer addToRunLoop:[NSRunLoop currentRunLoop] forMode:NSRunLoopCommonModes];
        }
    }
}
